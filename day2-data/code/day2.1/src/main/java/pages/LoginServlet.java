...
public class LoginServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		try {
			...
			//ServletException(String errMesg,Throwable rootcause)
	 ...
	 *      response)
	 */
	//1.st call public service then it further calls protected service as we want to enter the http world
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 1. set resp content type
		response.setContentType("text/html");
		
		// 2 get PW -ti send character response
		try (PrintWriter pw = response.getWriter()) {
			
			// 3. get req params(em, pass) sent from the clnt to sever
			...
			if (user == null) // login failed
				...
				//ceate  a cookie to hold user details
				//javax.servlet.http.Cookies(String nm,String val)
		...
		//		pw.flush();//sending the resp (commiting the resp)
				...
				if (user.isVotingStatus()) {// alrdy voted
					...
					//Wc will descard it will empty the buffer
					response.sendRedirect("candidate_list");
					//WC sends temp redirect resp : SC 302 | location=candidate_list,
//					SetCookie : user_dtls : user deatils.toString() 
//					| body : EMPTY
					//browser has generated the req so bydefault for browser is get method
